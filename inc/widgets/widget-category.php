...
class Business_world_categ extends WP_Widget
{
  function __construct()
  {
    $widget_ops = array('description' => __('Displays Categories Posts', "business-world"));
    $control_ops = array('width' => '', 'height' => '');
    parent::__construct(false, $name = __('Categories Posts', "business-world"), $widget_ops, $control_ops);
  }
  ...
  function widget($args, $instance)
  {
    ...
    while ($wp_query->have_posts()) : $wp_query->the_post(); ?>
      ...
      <div class="cat_widg_cont">
        ...
        <div class="cat_widg">
          <?php if (has_post_thumbnail() || (Business_world_frontend_functions::post_image_url() && $grab_image)) { ?>
            <a href="<?php the_permalink(); ?>">
              <div class="cat_widg-img">
                <div class="img_container unfixed ">
                  <?php echo Business_world_frontend_functions::auto_thumbnail($grab_image); ?>
                </div>
              </div>
            </a>
          <?php } ?>
          <p>
            <?php echo Business_world_frontend_functions::the_excerpt_max_charlength(100); ?>
          </p>
        </div>
  ...
  {
    //Defaults
    $instance = wp_parse_args((array)$instance, array('title' => __('Categories Posts', "business-world"), 'categ_id' => '0', 'post_count' => '3'));

    $title = esc_attr($instance['title']);
    $categ_id = esc_attr($instance['categ_id']);
    $post_count = esc_attr($instance['post_count']) ?>

    <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php echo __('Title', "business-world"); ?>:</label><input
        class="widefat" id="<?php echo $this->get_field_id('title'); ?>"
        name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>"/></p>


    <p><label for="<?php echo $this->get_field_id('categ_id'); ?>"><?php echo __('Select Category', "business-world"); ?>
        :</label>
      <select name="<?php echo $this->get_field_name('categ_id'); ?>" id="<?php echo $this->get_field_id('categ_id') ?>"
        ...
        <option value="0"><?php echo __('Select Category', "business-world"); ?></option>
    ...
    <p><label for="<?php echo $this->get_field_id('post_count'); ?>"><?php echo __('Number of Posts', "business-world"); ?>
...
add_action('widgets_init', create_function('', 'return register_widget("Business_world_categ");'))
?>