...
class Business_world_frontend_functions extends WDWT_frontend_functions
{
  ...
  public static function posted_on($thumb_class)
  {
    $time_string = '<time class="entry-date published updated" datetime="%1$s">%4$s</time>';
    if (get_the_time('U') !== get_the_modified_time('U')) {
      $time_string = '<time class="entry-date published" datetime="%1$s">%4$s</time><time class="updated" datetime="%2$s">%3$s</time>';
    }
    printf('<a href="%5$s" title="%6$s" rel="bookmark" class="date ' . $thumb_class . '">' . $time_string . '</a>',
  ...
  public static function posted_on_single()
  {
    ...
    if (get_the_time('U') !== get_the_modified_time('U')) {
      $time_string = '<time class="entry-date published" datetime="%3$s">%4$s</time><time class="updated" datetime="%8$s">%9$s</time>';
    }
    printf('<span class="date_link"><span class="sep date"></span><span class="screen-reader-text">' . __('Posted on', "business-world") . '</span><a href="%1$s" title="%2$s" rel="bookmark">' . $time_string . '</a></span><span class="by-author"> <span class="sep author"></span> <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>',
      esc_url(get_permalink()),
      esc_attr(get_the_time()),
      esc_attr(get_the_date(DATE_W3C)),
      esc_html(get_the_date()),
            ...
            <div class="featured-content clear-div">
              ...
              while ($abaut_us_post->have_posts()):
                ...
                if (has_post_thumbnail() || (self::post_image_url() && $wdwt_front->blog_style() && $wdwt_front->grab_image())) { ?>
                  <div class="home_about_us-thumb img_container fixed size300x200">
                    <?php echo self::fixed_thumbnail(300, 200, $wdwt_front->grab_image()); ?>
                  </div>
                <?php } ?>
        ...
        <div class="cont_horizontal_tab">
          <ul class="content">
            ...
            while ($h_tab_wp_query->have_posts()) : $h_tab_wp_query->the_post();
              $i++; ?>
              <li <?php if ($i == 1) echo "class='active'"; ?> id="horizontal-tabs-content-<?php echo get_the_ID(); ?>">
        ...
        <div id="tabs_div">
          ...
          <div id="tabs_content">
            <ul class="tabs clear-div">
              <?php
              while ($h_tab_wp_query->have_posts()) : $h_tab_wp_query->the_post(); ?>
                <li id="horizontal-tab-<?php echo $post->ID; ?>" class="img_container fixed size250x230">
        ...
        <div class="blog-post content-posts clear-div">
          ...
          if (have_posts()) {
            while (have_posts()) {
              ...
              <div class='content-post hentry'>
                <?php if (has_post_thumbnail() || (self::post_image_url() && $grab_image)) { ?>
                  <div class="content-thumb img_container fixed">
                ...
                <?php self::posted_on($thumb_class); ?>
  ...
  public static function content_posts()
  {
    ...
    global $post, $wdwt_front, $wp_query;
    $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
    $content_posts_enable = $wdwt_front->get_param('content_posts_enable', array(), true);
    $cat_checked = $wdwt_front->get_param('content_post_categories');
    ...
    if ($content_posts_enable && have_posts()) { ?>
