...
class WDWT_updater{
  ...
  */
  ...
  protected function json_to_string_meta($val){
    if(is_array($val)){
      $result = implode("||wd||", $val);
    } else{
       $val = json_decode( $val , true );
       if( !is_null($val) ){ 
        $result = implode("||wd||", $val);
  ...
  protected function boolean_to_array_slide( $val, $param_name, $args = array()){
  if(is_bool($val)){
    if ($val === true) {
      $this->options[$param_name] = array('Hide Slider');
    }
  ...
  protected function get_new_color_value( $val ){
      $result = '#'.$val;
    return $result;
  }
